...
| Full profile launcher | `scripts/run_full_profile.sh` | Dry-run verified locally on 2026-05-13 for `dino_imagenet_l11` and `ijepa_imagenet_l31`. It chains runtime/config/model/manifest gates, feature extraction, native and SAE probes, SAE-code extraction, Availability, Access, Allocation, artifact indexing, tables, and optional figures. | Currently supports the matched ImageNet vertical profiles. Additional layers, tasks, and SAE settings should extend this launcher or add sibling profiles under the same artifact contract. |
