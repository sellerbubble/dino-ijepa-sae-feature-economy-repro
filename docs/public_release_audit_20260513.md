...
| First real saved-array bundle plan | `docs/first_real_saved_array_bundle_plan.md` | Defines the public v1 model/task/SAE matrix, expected 149 run-plan rows after ranking-control, default layer-sweep, and Module F expansion, task-level staging profiles, acceptance gates, and an export checklist for the optional audit release asset. The earlier saved-array candidate remains documented as a pre-ranking-control `66/66` audit bundle. | Keep this document synchronized with future release asset layouts. |
...
| Standalone repo export | `scripts/export_public_repo.sh` | Copies the curated public slice into a clean standalone working tree, excludes caches/generated local artifact directories, and runs the release hygiene checker on the exported tree. Verified on 2026-05-13 by exporting to `/tmp`, running the public unit test suite, running `check_public_release.py`, and building the tiny artifact bundle from the exported tree. | Does not initialize git, push to GitHub, or attach real saved-array release assets. |
...
| Native probes | `probe-native --backend fixture/linear-probe/paper-scale-torch` | Unit-tested for classification, counting, depth, and segmentation saved arrays. `paper-scale-torch` trains AdamW probes on train arrays and selects checkpoints on validation metrics; `linear-probe` remains the lightweight diagnostic path. | Dense paper-scale outputs include compatibility weight proxies for existing ranking/ablation readers. |
| SAE-code probes | `probe-sae --backend fixture/linear-probe/paper-scale-torch` | Unit-tested for classification, counting, depth, and segmentation SAE-code arrays. The full-profile launcher now defaults to `paper-scale-torch` and preserves `linear-probe` for smoke/low-cost diagnostics. | Module E still uses the public linear/proxy readout interface for ablation until the decoder-native ablation module is ported. |
...
| Access | `rank-features`, `compute-contributions`, `compute-subset-usage`, `task_feature_ranking.json`, `contribution_scores.npz`, `subset_usage_summary.json` | Unit-tested and smoke-tested, including matched random controls. `rank-features` supports `probe_weight`, `validation_contribution`, and `hybrid` ranking methods; `compute-contributions` writes validation-contribution scores from the configured SAE-code probe outputs. The full-profile classification launcher defaults to scalable `true_class_logit_drop`, with `exact_metric_drop` available for small audit runs. | Full profiles consume `paper-scale-torch` probe outputs by default; `linear-probe` remains a diagnostic backend. |
| Allocation | `ablate-features --backend linear-probe`, `feature_ablation_summary.json` | Unit-tested for classification, depth, segmentation; smoke runs classification and dense-segmentation ablations. | Perturbation-response analysis is intentionally outside the public default path. |
| Module F native subspace ablation | `ablate-native-subspace`, `native_subspace_ablation_summary.json`, `configs/advanced/module_f_native_ablation_nyuv2.yaml` | Unit-tested on saved native arrays. The command removes SAE decoder-direction subspaces in SAE runtime-normalized hidden coordinates centered by `decoder_bias`, then re-encodes through the frozen SAE and evaluates the frozen SAE-code probe. | Requires lightweight SAE checkpoints with `decoder_weight`; raw-image perturbation analysis is not included. |
...
| Full reproduction guide | `docs/full_reproduction.md` | Defines the default full paper-style rerun path, required inputs, outputs, resource expectations, acceptance checks, and all eight validated model/task profiles. | Perturbation-response analysis remains outside the default public v1 path. |
