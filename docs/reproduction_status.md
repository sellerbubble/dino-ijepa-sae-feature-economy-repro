...
| `feature-economy probe-native` | fixture, linear-probe, and paper-scale-torch backends executable | Native probe output contract from fixture predictions or saved train/validation features plus labels/targets. | Yes for paper-style classification/counting/depth/segmentation probes; linear-probe remains the lightweight path. |
| `feature-economy probe-sae` | fixture, linear-probe, and paper-scale-torch backends executable | SAE-code probe summary from fixture predictions or saved train/validation codes plus labels/targets. | Yes for paper-style classification/counting/depth/segmentation probes; dense decoder outputs expose compatibility weight proxies for existing ranking/ablation readers. |
